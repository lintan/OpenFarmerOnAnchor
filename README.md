...
6. 输入私钥和密码（控制不显示输入是正常的，复制进去后直接【回车】）
...
7. 脚本多开，请把整个源码目录复制一份，在另一个目录中修改配置文件【user.yaml】为另一个账号，双击运行 【main.py】 启动第二个脚本，以此类推，多开互不干扰
8. 正确关闭程序，请点击脚本控制台窗口右上角的X，稍等几秒钟便会关闭，或者点击脚本控制台窗口后，按Ctrl+C
9. 重置私钥可以手动删除encrypttext文件里的加密串，也可以命令行运行python main.py clear
